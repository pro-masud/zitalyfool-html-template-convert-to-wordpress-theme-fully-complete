...
if (! function_exists('sc_table_of_contents')) {
	function sc_table_of_contents($attr, $content = null)
	{
		...
		$mfn_items = get_post_meta(mfn_ID(), 'mfn-page-items', true);
		...
		foreach(explode(' ', $tags_anchors) as $key => $value){
			...
			if ($highest_heading === 0) {
				$highest_heading = $value[1];
			}else if($highest_heading > $value[1]){
				$highest_heading = $value[1];
			}
		}
		$tags_regex .= '/i';
		...
		$mfn_headings = array();
		foreach($mfn_columns as $key => $value){
			preg_match_all( $tags_regex, $value, $matches );
			foreach($matches[0] as $in_key => $in_value){
				$mfn_headings[] =  wp_specialchars_decode($in_value);
			}
		}
		...
		$nested_depth = 1;
		$custom_icon = $marker_view === 'bullets' ? 'class="'.$icon.'"' : '';
				...
				$output .= '<div class="table_of_content_wrapper">';
					$output .= '<ol class="mfn_toc_'. $marker_view .'">';
						...
						foreach($mfn_headings as $key => $value){
							...
							// prev heading > new heading = new ol
							// prev heading === new heading == (if nested ol, close), create new li

							switch(true){

								case $previous_heading_level === 0: //init
									$output .= '<li '.$custom_icon.' ><a class="scroll" href="#toc_'.$key.'">'. sanitize_text_field($value) .'</a></li>';
									break;
								...
								case $previous_heading_level < $matches[0] && $nested_depth <= 2: //increase nest
									$nested_depth++;

									$output .= '<li class="mfn_toc_nested"><ol>';
										$output .= '<li '. $custom_icon .'><a class="scroll" href="#toc_'.$key.'"> '.sanitize_text_field($value).' </a></li>';
									break;

								case $previous_heading_level > $matches[0] && $nested_depth >= 0: //decrease nest
									$nested_depth--;
